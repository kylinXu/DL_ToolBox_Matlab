...
dnn = cpNNtoGPU(hnn,cast);   % COPY NETWORK hnn TO DEVICE dnn
...
for i = 1 : numepochs
    ...
    for l = 1 : numbatches
        ...
        % COPY BATCHES hbatch_x TO GPU DEVICE dbatch_x
        ...
        L(n) = gather(dnn.L); % 扫描（迭代）一次，L为每一次batch迭代的损失函数值
        n = n + 1;
    end
    ...
    if ishandle(fhandle)
        ...
        opts.plotfun(dnn, fhandle, hloss, opts, i);
    ...
    switch dnn.output
        case {'sigm', 'linear'}
            disp(['epoch ' num2str(i) '/' num2str(opts.numepochs)  ...
                '. Took ' num2str(t) ' seconds' '. MSE on training set is '...
        ...
        case 'softmax'
            disp(['epoch ' num2str(i) '/' num2str(opts.numepochs)  ...
                '. Took ' num2str(t) ' seconds' '. MCE on training set is '...
                num2str(mean(L((n-numbatches):(n-1))))])
            disp(['         Eval time: ' num2str(t2) ...
                '. LearningRate: ', num2str(hnn.learningRate) '.Momentum : ' num2str(hnn.momentum)...
                '. free gpu mem (Gb): ', num2str(gpu.FreeMemory./10^9)]);
